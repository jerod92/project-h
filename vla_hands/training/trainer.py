...
# ── CurriculumConfig + TrainingCurriculum ────────────────────────────────────
...
class CurriculumConfig:
    ...
    Single config object for the full BC → RL pipeline.  Pass to
    ``TrainingCurriculum`` or ``auto_curriculum`` instead of managing
    ``TrainerConfig`` directly.
    """
...
class TrainingCurriculum:
    ...
    Sequences BC (warm-start from expert) then RL (environment reward).
